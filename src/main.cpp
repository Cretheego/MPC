    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
	      ...
	      vector<double> x_points = ptsx;
          vector<double> y_points = ptsy;
		  //1.Transform the global waypoints to vehicle coordinates

		  Eigen::VectorXd ptsxVals = Eigen::VectorXd(ptsx.size());
          Eigen::VectorXd ptsyVals = Eigen::VectorXd(ptsy.size());
		  for(int i = 0; i < ptsx.size(); i++) {
			x_points[i] = (ptsx[i] - px)*cos(psi) + (ptsy[i] - py)*sin(psi);
          }
					
          for(int i = 0; i<ptsy.size(); i++){
			y_points[i] = (ptsy[i]- py)*cos(psi) - (ptsx[i] - px)*sin(psi);
		  }
		  //2.Fit the p lynomial to the waypoints.
          
          for (int i=0; i<ptsx.size(); i++) {
 			 ptsxVals[i] = x_points[i];//ptsx[i];
             ptsyVals[i] = y_points[i];//ptsy[i];
          }  
		  ...
		  */
          ...
          double latency_y = 0;
          double latency_psi = -(v / Lf) * steer_value * latency_dt;
          double latency_v = v + throttle_value * latency_dt;
          ...
          msgJson["next_x"] = next_x;//x_points;//ptsx;
          msgJson["next_y"] = next_y;//y_points;//ptsy;
